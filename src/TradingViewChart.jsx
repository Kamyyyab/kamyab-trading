...
const QUICK = [
  ['MYM1!','MYM'],['MNQ1!','MNQ'],['MES1!','MES'],
  ['YM1!','YM'],['NQ1!','NQ'],['ES1!','ES'],
  ['GC1!','Gold'],['CL1!','Oil'],['BTC1!','BTC'],
]
const TFS = [
...
const isFutures = s => /1!$/.test(s) || /^(CBOT:|CME:|COMEX:|NYMEX:)/.test(s)
...
export default function TradingViewChart() {
  ...
  // Poll prices for non-futures alerts
  useEffect(() => {
    const active = alerts.filter(a => !a.triggered && !isFutures(a.symbol))
  ...
  function addAlert() {
    ...
    const a = {
      ...
      dir:       aDir,
      label:     aLabel || `${aSym.toUpperCase()} ${aDir==='above'?'↑':'↓'} ${aPrice}`,
      triggered: false,
  ...
  const inp9 = { background:'#08101c', border:'1px solid #1c2e4a', borderRadius:'6px', color:'#dce8f5', fontFamily:M, fontSize:'11px', padding:'7px 10px', outline:'none', boxSizing:'border-box', width:'100%' }
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
        ...
        <div style={{ padding:'10px 14px', display:'flex', gap:'8px', alignItems:'center', flexWrap:'wrap', borderBottom:'1px solid #162340', background:'#0a1020' }}>
          ...
          <div style={{ display:'flex', gap:'4px', alignItems:'center', flexShrink:0 }}>
            <input
              ...
              style={{ width:'140px', background:'#070a14', border:'1px solid #1c2e4a', borderRadius:'5px', color:'#dce8f5', fontFamily:M, fontSize:'10px', padding:'5px 9px', outline:'none' }}
            />
          ...
          <div style={{ display:'flex', gap:'3px', flexWrap:'wrap' }}>
            {QUICK.map(([v,l]) => (
              <button key={v} onClick={() => { setSym(v); setSymInput(v) }} style={{
                fontFamily:M, fontSize:'8px', padding:'4px 7px', borderRadius:'4px',
                border:`1px solid ${sym===v?'rgba(245,158,11,0.4)':'#162340'}`,
                background:sym===v?'#18100a':'transparent',
                color:sym===v?'#f59e0b':'#6880a0', cursor:'pointer',
                WebkitTapHighlightColor:'transparent',
              }}>{l}</button>
            ))}
          </div>

          {/* Timeframes — push to right */}
      ...
      <div style={{ background:'#0c1422', border:'1px solid #162340', borderRadius:'12px', padding:'16px' }}>
        <div style={{ fontFamily:M, fontSize:'8px', color:'#7a96b4', letterSpacing:'2px', marginBottom:'14px' }}>PRISNIVÅLARM</div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr auto', gap:'8px', marginBottom:'8px' }}>
          <div>
            ...
            <input value={aSym} onChange={e=>setASym(e.target.value.toUpperCase())} placeholder="AAPL, ETH, SPY..." style={inp9} />
          </div>
          <div>
        ...
        <div style={{ fontFamily:M, fontSize:'8px', color:'#2a3c50', marginBottom:'10px' }}>
          Aktier, index och krypto: auto-notis var 30s. Futures (MYM1! etc): använd TradingView-larmet istället.
        </div>
        ...
        {alerts.map(a => {
          ...
          const curPrice = prices[a.symbol]
          return (
            <div key={a.id} style={{
              ...
              <div style={{ fontFamily:M, fontSize:'14px', color:a.dir==='above'?'#00e5b0':'#ff4f6b', flexShrink:0 }}>
                {a.dir==='above'?'↑':'↓'}
              </div>
              <div style={{ flex:1, minWidth:0 }}>
                ...
                <div style={{ fontFamily:M, fontSize:'8px', color:'#4a6888', marginTop:'2px' }}>
                  {a.triggered
                    ...
                    : fut
                      ? 'Futures — lägg larm i TV-chartet'
                      : curPrice
                        ? `Nu: ${curPrice.toFixed(2)} — ${Math.abs(((curPrice-a.price)/a.price)*100).toFixed(1)}% kvar`
                        : 'Kontrollerar...'}
                </div>
              ...
              <button onClick={() => setAlerts(p=>p.filter(x=>x.id!==a.id))} style={{ background:'none', border:'none', color:'#3a5878', cursor:'pointer', fontSize:'18px', padding:'0 2px', flexShrink:0, lineHeight:1 }}>×</button>
            </div>
          )
        })}
      </div>
