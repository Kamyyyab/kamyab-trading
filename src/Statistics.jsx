...
export default function Statistics({ journal = [] }) {
  ...
  const wTrades = trades.filter(t=>t.result==='win'||t.result==='win2')
  ...
  const totalR = trades.reduce((s,t)=>{ if(t.result==='win')return s+3; if(t.result==='win2')return s+2; if(t.result==='loss')return s-1; return s },0)
  ...
  const setupStats = {}
  trades.forEach(t=>{ const k=t.setup||'Otaggad'; if(!setupStats[k])setupStats[k]={pnl:0,wins:0,total:0}; setupStats[k].pnl+=parseFloat(t.pnl||0); setupStats[k].total++; if(t.result==='win'||t.result==='win2')setupStats[k].wins++ })
  const setupList = Object.entries(setupStats).sort(([,a],[,b])=>b.pnl-a.pnl)

  const psychStats = {}
  trades.forEach(t=>{ (t.psychTags||[]).forEach(id=>{ if(!psychStats[id])psychStats[id]={wins:0,losses:0,total:0,pnl:0}; psychStats[id].total++; psychStats[id].pnl+=parseFloat(t.pnl||0); if(t.result==='win'||t.result==='win2')psychStats[id].wins++; if(t.result==='loss')psychStats[id].losses++ }) })
