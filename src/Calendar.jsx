      ...
      <div>
        ...
        <div style={{ display:'flex', flexWrap:'wrap', gap:'5px' }}>
          {PSYCH.map(tag => {
            ...
            return <button key={tag.id} onClick={() => toggle(tag.id)} style={{ fontFamily:M, fontSize:'9px', padding:'5px 10px', borderRadius:'5px', background:a?tag.bg:'#161e24', border:`1px solid ${a?tag.c+'33':'#1e2c32'}`, color:a?tag.c:'#85a4ad', cursor:'pointer', transition:'all 0.15s' }}>{tag.label}</button>
          })}
        </div>
      </div>
      <div>
      ...
      <div>
        ...
        {!imgPrev ? (
          <button type="button" onClick={() => fileRef.current?.click()} style={{
            width:'100%', background:'#080b0c', border:'1px dashed #263840', borderRadius:'8px',
            color:'#85a4ad', fontFamily:M, fontSize:'10px', padding:'14px', cursor:'pointer',
            transition:'all 0.15s', letterSpacing:'0.5px',
          }}
            onMouseEnter={e=>{e.currentTarget.style.borderColor='#3a5460';e.currentTarget.style.color='#a0c0ca'}}
            onMouseLeave={e=>{e.currentTarget.style.borderColor='#263840';e.currentTarget.style.color='#85a4ad'}}
          >+ Ladda upp bild</button>
        ) : (
          <div style={{ position:'relative' }}>
            <img src={imgPrev} alt="preview" style={{ width:'100%', borderRadius:'8px', border:'1px solid #263840', display:'block' }} />
            <button type="button" onClick={clearImg} style={{
              position:'absolute', top:'6px', right:'6px', background:'rgba(6,8,9,0.85)',
              border:'1px solid #263840', borderRadius:'5px', color:'#ff4f6b',
              fontFamily:M, fontSize:'10px', padding:'3px 8px', cursor:'pointer',
            }}>✕ Ta bort</button>
          </div>
        )}
      </div>
      <div style={{ display:'flex', gap:'8px' }}>
        <button onClick={() => onSave({ result, instrument:instr, pnl:pnl||'0', note, emotion, setup, psychTags:tags, image:image||null })}
        ...
        <button onClick={onCancel} style={{ background:'transparent', color:'#85a4ad', fontFamily:M, fontSize:'10px', padding:'12px 14px', borderRadius:'8px', border:'1px solid #1e2c32', cursor:'pointer' }}>Avbryt</button>
      </div>
    </div>
  )
}
...
export default function Calendar({ journal=[], onAddTrade, onDeleteTrade, onEditTrade }) {
  ...
  function resetForm() { setShowForm(false) }
  ...
  const panelContent = (
    <div style={{ display:'flex', flexDirection:'column', height:'100%', overflow:'hidden' }}>
      <div style={{ padding:'14px', borderBottom:'1px solid #1e2c32', display:'flex', justifyContent:'space-between', alignItems:'flex-start', flexShrink:0 }}>
        <div>
          <div style={{ fontFamily:M, fontSize:'8px', color:'#85a4ad', letterSpacing:'2px', marginBottom:'3px' }}>
        ...
        <div style={{ display:'flex', gap:'6px', alignItems:'center' }}>
          ...
          <button onClick={() => { setSel(null); resetForm(); setEditingIdx(null) }} style={{ background:'none', border:'none', color:'#85a4ad', cursor:'pointer', fontSize:'18px', padding:'4px', lineHeight:1 }}>×</button>
        </div>
      </div>
      ...
      <div style={{ overflowY:'auto', flex:1, minHeight:0, padding:'12px', display:'flex', flexDirection:'column', gap:'10px' }}>
        {showForm && (
          <div style={{ background:'#080b0c', border:'1px solid #263840', borderRadius:'10px', padding:'14px' }}>
            <div style={{ fontFamily:M, fontSize:'8px', color:'#85a4ad', letterSpacing:'2px', marginBottom:'10px' }}>NY TRADE</div>
            <EditForm
        ...
        {selTrades.length===0 && !showForm && (
          <div style={{ fontFamily:M, fontSize:'11px', color:'#4a6470', padding:'20px 0', textAlign:'center' }}>Inga trades — tryck + LOG</div>
        )}
            ...
            <div key={i} style={{ background:'#080b0c', border:`1px solid ${RBDR[t.result]||'#1e2c32'}`, borderRadius:'10px', overflow:'hidden' }}>
              <div style={{ padding:'11px 12px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                <div style={{ display:'flex', alignItems:'center', gap:'6px', flexWrap:'wrap' }}>
                  ...
                  <span style={{ fontFamily:M, fontSize:'9px', padding:'2px 7px', borderRadius:'4px', background:RBG[t.result]||'#111820', color:RC[t.result]||'#85a4ad', fontWeight:600 }}>{RL[t.result]||t.result}</span>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
                  ...
                  <button onClick={() => setEditingIdx(isEd?null:ji)} style={{ background:isEd?'#263840':'none', border:`1px solid ${isEd?'#3a5460':'#1e2c32'}`, borderRadius:'5px', color:isEd?'#d0e8ec':'#85a4ad', fontFamily:M, fontSize:'9px', padding:'3px 8px', cursor:'pointer', transition:'all 0.15s' }}>✎</button>
                  <button onClick={() => onDeleteTrade?.(ji)} style={{ background:'none', border:'none', color:'#5a7a84', cursor:'pointer', fontSize:'14px', padding:'2px', transition:'color 0.15s' }}
                    onMouseEnter={e=>e.currentTarget.style.color='#ff4f6b'}
                    onMouseLeave={e=>e.currentTarget.style.color='#5a7a84'}>×</button>
                </div>
              </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'8px' }}>
        {[
          ...
          <div key={i} style={{ background:'#111820', border:'1px solid #1e2c32', borderRadius:'10px', padding:'10px 12px' }}>
            <div style={{ fontFamily:M, fontSize:'7px', color:'#85a4ad', letterSpacing:'1px', marginBottom:'3px' }}>{s.l}</div>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        ...
        <div style={{ display:'flex', gap:'5px' }}>
          {[{fn:prevM,l:'‹'},{fn:nextM,l:'›'}].map((b,i) => (
            <button key={i} onClick={b.fn} style={{ background:'#111820', border:'1px solid #1e2c32', borderRadius:'6px', padding:'8px 16px', cursor:'pointer', color:'#88a8ae', fontFamily:M, fontSize:'14px', transition:'all 0.15s' }}
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ background:'#111820', border:'1px solid #1e2c32', borderRadius:'12px', overflow:'hidden' }}>
            <div style={{ display:'grid', gridTemplateColumns:gridColsHdr, background:'#0d1214', borderBottom:'1px solid #1e2c32' }}>
              {dayLabels.map((d,i) => (
                <div key={i} style={{ fontFamily:M, fontSize:'9px', color:'#85a4ad', textAlign:'center', padding:'10px 4px', letterSpacing:'1px' }}>{d}</div>
              ))}
              {!mobile && <div style={{ fontFamily:M, fontSize:'8px', color:'#5a7a84', textAlign:'center', padding:'10px 4px', borderLeft:'1px solid #1e2c32' }}>V</div>}
            </div>
                    ...
                    return (
                      <div key={ci}
                        ...
                        style={{
                          ...
                          outlineOffset:'-2px', cursor:'pointer', position:'relative', transition:'background 0.15s',
                        }}>
                        {isTd && <div style={{ position:'absolute', top:0, left:0, right:0, height:'2px', background:'#007d5e' }} />}
                        <div style={{ fontFamily:M, fontSize:'11px', color:isTd?'#00e5b0':'#a0c0ca', fontWeight:isTd?700:400, marginBottom:'4px' }}>{day}</div>
                        {!mobile && aT.map((t,ti) => (
                          <div key={ti} style={{ fontFamily:M, fontSize:'8px', padding:'2px 5px', borderRadius:'3px', background:RBG[t.result]||'#111820', color:RC[t.result]||'#85a4ad', display:'inline-block', marginBottom:'2px', marginRight:'2px' }}>
                    ...
                    <div style={{ minHeight:cellH, borderBottom:'1px solid #161e24', borderLeft:'1px solid #1e2c32', padding:'8px 6px', display:'flex', flexDirection:'column', justifyContent:'center', alignItems:'center', gap:'2px', background:'#0d1214' }}>
                      {wT.length>0 ? (
                        <>
                          ...
                          <div style={{ fontFamily:M, fontSize:'7px', color:'#85a4ad' }}>{wT.length}t</div>
                        </>
