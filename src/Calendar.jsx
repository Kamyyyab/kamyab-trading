    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100%' }}>
      <div style={{ padding:'14px', borderBottom:'1px solid #1e2c32', display:'flex', justifyContent:'space-between', alignItems:'flex-start', flexShrink:0 }}>
        ...
        <div style={{ display:'flex', gap:'6px', alignItems:'center' }}>
          <button type="button" onClick={() => { setShowForm(!showForm); setEditingIdx(null) }} style={{ background:'#00e5b0', color:'#020f08', fontFamily:M, fontSize:'9px', fontWeight:700, padding:'7px 13px', borderRadius:'6px', border:'none', cursor:'pointer', letterSpacing:'1px', transition:'background 0.15s' }}>+ LOG</button>
          <button type="button" onClick={() => { setSel(null); setShowForm(false); setEditingIdx(null) }} style={{ background:'none', border:'none', color:'#5a7a84', cursor:'pointer', fontSize:'18px', padding:'4px', lineHeight:1 }}>×</button>
        </div>
      </div>
              ...
              return (
                <div key={wi} style={{ display:'grid', gridTemplateColumns:gridCols }}>
                  {week.map((cell, ci) => {
                    ...
                    return (
                      <div key={ci}
                        onClick={() => { setSel(isSel?null:ds); setShowForm(false); setEditingIdx(null) }}
                        style={{
                          minHeight:cellH,
