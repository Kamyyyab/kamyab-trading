...
const RBDR = {
  win: 'rgba(0,229,176,0.2)',
  win2: 'rgba(0,229,176,0.2)',
  tp1: 'rgba(74,184,154,0.2)',
  tp2: 'rgba(0,229,176,0.2)',
  tp3: 'rgba(0,229,176,0.2)',
  loss: 'rgba(255,79,107,0.2)',
  be: 'rgba(106,138,144,0.15)',
  skip: '#1e2c32',
  'no-setup': '#1e2c32',
}
...
  const [image, setImage] = useState(initial.image || initial.image_url || null)
...
                  padding: '5px 10px',
                  borderRadius: '5px',
                  background: a ? tag.bg : '#161e24',
                  border: `1px solid ${a ? tag.c + '33' : '#1e2c32'}`,
                  color: a ? tag.c : '#5a7a84',
                  cursor: 'pointer',
                  transition: 'all 0.15s',
                }}
              >
...
  const allM = journal.filter(t => {
    const d = new Date(t.date)
    return d.getFullYear() === year && d.getMonth() === month
  })

  const mT = allM.filter(t => t.result !== 'skip' && t.result !== 'no-setup')
  const mW = mT.filter(t => t.result === 'win' || t.result === 'win2' || t.result === 'tp2' || t.result === 'tp3').length
...
    const hw = act.some(
      t => t.result === 'win' || t.result === 'win2' || t.result === 'tp2' || t.result === 'tp3'
    )
...
    if (status === 'win') {
      return {
        bg: `rgba(0,40,30,${Math.min(0.2 + Math.abs(pnl) / 1500, 0.5)})`,
        bdr: 'rgba(0,229,176,0.25)',
        dot: '#00e5b0',
      }
    }
...
    if (status === 'loss') {
      return {
        bg: `rgba(40,0,10,${Math.min(0.2 + Math.abs(pnl) / 800, 0.5)})`,
        bdr: 'rgba(255,79,107,0.25)',
        dot: '#ff4f6b',
      }
    }
...
          {selTrades.filter(t => t.result !== 'skip' && t.result !== 'no-setup')
            .length > 0 && (
            <div
              style={{
                fontFamily: M,
...
                      fontSize: '14px',
                      fontWeight: 700,
                      color: pv >= 0 ? '#00e5b0' : '#ff4f6b',
                    }}
                  >
                    {pv >= 0 ? '+' : ''}${Math.abs(Math.round(pv))}
                  </span>

                  <button
                    type="button"
                    onClick={() => setEditingIdx(isEd ? null : ji)}
                    style={{
                      background: isEd ? '#263840' : 'none',
                      border: `1px solid ${isEd ? '#3a5460' : '#1e2c32'}`,
                      borderRadius: '5px',
                      color: isEd ? '#d0e8ec' : '#5a7a84',
                      fontFamily: M,
                      fontSize: '9px',
                      padding: '3px 8px',
                      cursor: 'pointer',
...
                  <EditForm
                    initial={t}
                    onSave={updated => doEdit(ji, updated)}
...
                              height: '3px',
                              borderRadius: '2px',
                              background: n <= em ? ec : '#1e2c32',
                            }}
                          />
                        ))}
                      </div>

                      <span style={{ fontFamily: M, fontSize: '9px', color: ec, flexShrink: 0 }}>
                        {em}/10
                      </span>
                    </div>
                  )}
...
          {
            l: 'P&L',
            v: `${mPnl >= 0 ? '+' : ''}$${Math.round(mPnl)}`,
            c: mPnl >= 0 ? '#00e5b0' : '#ff4f6b',
          },
          {
            l: 'WIN RATE',
            v: `${mWr}%`,
...
          {
            l: 'SNITT/DAG',
            v: `${avgDay >= 0 ? '+' : ''}$${Math.round(avgDay)}`,
            c: avgDay >= 0 ? '#00e5b0' : '#ff4f6b',
          },
        ].map((s, i) => (
          <div
            key={i}
...
              style={{
                display: 'grid',
                gridTemplateColumns: gridColsHdr,
                background: '#0d1214',
                borderBottom: '1px solid #1e2c32',
              }}
            >
...
              const wT = journal.filter(
                t => wDates.includes(t.date) && t.result !== 'skip' && t.result !== 'no-setup'
              )
...
                          background: isSel
                            ? sc?.bg || 'rgba(30,44,50,0.4)'
                            : sc?.bg || (isTd ? 'rgba(0,229,176,0.03)' : 'transparent'),
...
                              fontSize: '10px',
                              fontWeight: 700,
                              color: wPnl >= 0 ? '#00e5b0' : '#ff4f6b',
                            }}
                          >
                            {wPnl >= 0 ? '+' : ''}${Math.abs(Math.round(wPnl))}
                          </div>
...
                          <div style={{ fontFamily: M, fontSize: '7px', color: '#5a7a84' }}>
                            {wT.length}t
                          </div>
                        </>
                      ) : (
                        <div style={{ fontFamily: M, fontSize: '10px', color: '#1e2c32' }}>
                          —
                        </div>
                      )}
                    </div>
