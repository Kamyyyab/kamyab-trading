...
export default function StreakHistory({ journal=[], onSaveStreakLog, streakLogs={} }) {
  ...
  const SBG  = {clean:'#001810',violation:'#1a0610',empty:'#0a1020'}
  const SBDR = {clean:'rgba(0,229,176,0.2)',violation:'rgba(255,79,107,0.2)',empty:'#162340'}
    ...
    <div style={{background:'#0f1828',border:'1px solid #162340',borderRadius:'12px',padding:'14px'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:'2px',marginBottom:'2px'}}>
        {['M','T','O','T','F','L','S'].map((l,i)=>(
          <div key={i} style={{fontFamily:M,fontSize:'7px',color:i>=5?'#162340':'#1c2e4a',textAlign:'center',padding:'2px 0'}}>{l}</div>
        ))}
      </div>
          ...
          <div key={wi} style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:'2px'}}>
            {week.map((day,di)=>{
              ...
              return (
                ...
                >
                  <div style={{fontFamily:M,fontSize:'9px',fontWeight:day.isToday?700:400,color:day.isToday?'#00e5b0':day.isWe?'#162340':'#7a96b4',lineHeight:1}}>{day.day}</div>
                  {hasPnl ? (
        ...
        <>
          ...
          <div style={{position:'fixed',left:Math.min(popup.x,window.innerWidth-210),top:Math.min(popup.y,window.innerHeight-160),zIndex:99,background:'#0f1828',border:'1px solid #1c2e4a',borderRadius:'10px',padding:'12px',minWidth:'180px',boxShadow:'0 16px 40px rgba(0,0,0,0.8)'}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'5px'}}>
              <button onClick={()=>logDay(popup.date,'clean')} style={{background:popup.status==='clean'?'#001810':'#0a1020',border:`1px solid ${popup.status==='clean'?'rgba(0,229,176,0.3)':'#1c2e4a'}`,borderRadius:'6px',color:'#00e5b0',fontFamily:M,fontSize:'10px',padding:'7px 10px',cursor:'pointer',textAlign:'left',transition:'all 0.15s'}}>✓ Följde regler</button>
              <button onClick={()=>logDay(popup.date,'violation')} style={{background:popup.status==='violation'?'#1a0610':'#0a1020',border:`1px solid ${popup.status==='violation'?'rgba(255,79,107,0.3)':'#1c2e4a'}`,borderRadius:'6px',color:'#ff4f6b',fontFamily:M,fontSize:'10px',padding:'7px 10px',cursor:'pointer',textAlign:'left',transition:'all 0.15s'}}>✕ Bröt regler</button>
              {popup.status!=='empty'&&<button onClick={()=>logDay(popup.date,null)} style={{background:'none',border:'1px solid #162340',borderRadius:'6px',color:'#7a96b4',fontFamily:M,fontSize:'8px',padding:'5px 10px',cursor:'pointer',textAlign:'left'}}>Rensa</button>}
            </div>
          </div>
        </>
      )}
