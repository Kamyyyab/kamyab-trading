...
function StreakHistory({ journal, onSaveStreakLog, streakLogs = {} }) {
  ...
  const calendarDays = useMemo(() => {
    ...
    for (let d = 1; d <= lastDay.getDate(); d++) {
      ...
      days.push({ date: dateStr, day: d, status, isWeekend, isFuture, isToday })
    }
    return days
  }, [viewYear, viewMonth, journal, streakLogs, todayStr])
    ...
    <div style={{ background: '#161c1e', border: '1px solid #1e2d31', borderRadius: '12px', padding: '14px 16px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <div style={{ fontFamily: 'monospace', fontSize: '8px', color: '#4a6870', letterSpacing: '2px' }}>STREAK</div>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: '3px' }}>
            <span style={{ fontFamily: 'monospace', fontSize: '20px', fontWeight: 700, color: currentStreak > 0 ? '#00e5b0' : '#4a6870', lineHeight: 1 }}>{currentStreak}</span>
            <span style={{ fontFamily: 'monospace', fontSize: '8px', color: '#4a6870' }}>d</span>
          </div>
        </div>
        <div style={{ display: 'flex', gap: '12px' }}>
          {[
            ...
            <div key={i} style={{ textAlign: 'center' }}>
              ...
              <div style={{ fontFamily: 'monospace', fontSize: '13px', fontWeight: 700, color: s.color }}>{s.value}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
        ...
        <div style={{ display: 'flex', gap: '4px' }}>
          <button onClick={prevMonth} style={{ background: '#1c2426', border: '1px solid #26383d', borderRadius: '5px', color: '#8aacb0', fontSize: '11px', width: '22px', height: '22px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>‹</button>
          <button onClick={nextMonth} style={{ background: isNextDisabled ? 'transparent' : '#1c2426', border: '1px solid #26383d', borderRadius: '5px', color: isNextDisabled ? '#2e4448' : '#8aacb0', fontSize: '11px', width: '22px', height: '22px', cursor: isNextDisabled ? 'default' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>›</button>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
        {weeks.map((week, wi) => (
          <div key={wi} style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '2px' }}>
            {week.map((day, di) => {
              if (!day) return <div key={di} style={{ borderRadius: '4px', height: '32px' }} />
              ...
              return (
                ...
                >
                  <div style={{ fontFamily: 'monospace', fontSize: '9px', fontWeight: day.isToday ? 700 : 400, color: day.isToday ? '#00e5b0' : day.isWeekend ? '#1e2d31' : '#4a6870', lineHeight: 1 }}>{day.day}</div>
      ...
      {popup && (
        <>
          ...
          <div style={{ position: 'fixed', left: Math.min(popup.x, window.innerWidth - 210), top: Math.min(popup.y, window.innerHeight - 160), zIndex: 99, background: '#1c2426', border: '1px solid #26383d', borderRadius: '10px', padding: '12px', minWidth: '180px', boxShadow: '0 8px 32px rgba(0,0,0,0.6)' }}>
